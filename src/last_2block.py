...
model_save_path = r'D:\Kuliah\Sem 7\TA1\CODE TA\model\model_last_2blocks.pt'
...
# === MODEL LAST 2 BLOCKS ===
class DualInputRULALast2Blocks(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # FREEZE SEMUA LAYER KECUALI LAYER3 DAN LAYER4 (LAST 2 BLOCKS)
        for param in self.resnet.parameters():
            param.requires_grad = False
        
        # UNFREEZE LAST 2 BLOCKS (layer3 dan layer4)
...
# === TRAINING LOOP - LAST 2 BLOCKS ===
def train_last_2blocks():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    print(f"🔧 Training on: {device} | FINETUNE_MODE=last_2blocks")
    ...
    model = DualInputRULALast2Blocks().to(device)
    ...
    plt.title(f"Confusion Matrix - Last 2 Blocks\nTest Accuracy: {test_acc:.2f}%")
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('LAST 2 BLOCKS - Loss per Epoch')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('LAST 2 BLOCKS - Validation Accuracy per Epoch')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    test_accuracy = train_last_2blocks()
    
    print("\n" + "="*70)
    print(f"LAST 2 BLOCKS TRAINING COMPLETED!")
