...
model_save_path = r'D:\Kuliah\Sem 7\TA1\CODE TA\model\model_last_3blocks.pt'
...
# === TRAINING LOOP - LAST 3 BLOCKS ===
def train_last_3blocks():
    ...
    plt.title(f"Confusion Matrix - Last 3 Blocks\nTest Accuracy: {test_acc:.2f}%")
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('LAST 3 BLOCKS - Loss per Epoch')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('LAST 3 BLOCKS - Validation Accuracy per Epoch')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    test_accuracy = train_last_3blocks()
    
    print("\n" + "="*70)
    print(f"LAST 3 BLOCKS TRAINING COMPLETED!")
