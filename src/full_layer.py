...
model_save_path = r'D:\Kuliah\Sem 7\TA1\CODE TA\model\model_full_layer.pt'
...
# === MODEL FULL LAYER ===
class DualInputRULAFullLayer(nn.Module):
    def __init__(self):
        super().__init__()
        
        # Load pre-trained ResNet18 - SEMUA LAYER DILATIH
        self.resnet = resnet18(weights=ResNet18_Weights.DEFAULT)
        
        # SEMUA PARAMETER DAPAT DILATIH (default requires_grad=True)
...
# === TRAINING LOOP - FULL LAYER ===
def train_full_layer():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    print(f"🔧 Training on: {device} | FINETUNE_MODE=full_layer")
    ...
    plt.title(f"Confusion Matrix - Full Layer\nTest Accuracy: {test_acc:.2f}%")
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('FULL LAYER - Loss per Epoch')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('FULL LAYER - Validation Accuracy per Epoch')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    test_accuracy = train_full_layer()
    
    print("\n" + "="*70)
    print(f"FULL LAYER TRAINING COMPLETED!")
